    ...
    public void sendLink(long chatId, Link link) {
        webClient
                ...
                .onStatus(
                        HttpStatusCode::is4xxClientError,
                        response -> Mono.error(
                                new ClientException("Ошибка клиента при отправке ссылки: " + response.statusCode())))
        ...
        webClient
                ...
                .onStatus(
                        ...
                        response -> Mono.error(new ClientException(
                                "Ошибка клиента при добавлении пользователя: " + response.statusCode())))
                .bodyToMono(String.class)
    ...
    public void deleteLink(long chatId, Link link) {
        webClient
                ...
                .onStatus(
                        HttpStatusCode::is4xxClientError,
                        response -> Mono.error(
                                new ClientException("Ошибка клиента при удалении ссылки: " + response.statusCode())))
                .bodyToMono(String.class)
                .subscribe(
