    ...
    public void getCommitMessages(String owner, String repo, String since, String link, long chatId) {
        webClient
            ...
            .uri(uriBuilder -> uriBuilder
                ...
                .build(owner, repo))
            .retrieve()
            .onStatus(HttpStatusCode::isError, response -> Mono.error(new APIException("Ошибка в работе с API" + response.statusCode())))
            .bodyToFlux(GitHubCommit.class)
            .map(GitHubCommit::getMessage)
            .collectList()
            .subscribe(response ->
                    botClient.sendUpdate(chatId, new Update(link, response)),
                error -> log.error(error.getMessage())); // что потом делать с этим списком
    }
